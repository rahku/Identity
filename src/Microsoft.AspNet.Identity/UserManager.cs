...
namespace Microsoft.AspNet.Identity
{
    ...
    public class UserManager<TUser> : IDisposable where TUser : class
    {
        ...
        public UserManager(IUserStore<TUser> store,
            ...
            Logger<UserManager<TUser>> logger,
        ...
        /// </value>
        protected internal virtual Logger<UserManager<TUser>> Logger { get; set; }
        ...
        public virtual async Task<bool> CheckPasswordAsync(TUser user, string password)
        {
            ...
            if (!success)
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "PasswordMismatch", new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<IdentityResult> AddPasswordAsync(TUser user, string password)
        {
            ...
            if (hash != null)
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "UserAlreadyHasPassword", new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<IdentityResult> AddLoginAsync(TUser user, UserLoginInfo login)
        {
            ...
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "LoginAlreadyAssociated", new { userId = await GetUserIdAsync(user) });
        ...
        {
            Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "UserAlreadyInRole", new { userId = await GetUserIdAsync(user), role = role });
        ...
        {
            Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "UserNotInRole", new { userId = await GetUserIdAsync(user), role = role });
        ...
        public virtual async Task<IdentityResult> ChangePhoneNumberAsync(TUser user, string phoneNumber, string token)
        {
            ...
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "InvalidToken", new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<bool> VerifyUserTokenAsync(TUser user, string tokenProvider, string purpose, string token)
        {
            ...
            if (!result)
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "InvalidToken", new { userId = await GetUserIdAsync(user), purpose = purpose });
        ...
        public virtual async Task<IdentityResult> SetLockoutEndDateAsync(TUser user, DateTimeOffset? lockoutEnd)
        {
            ...
            if (!await store.GetLockoutEnabledAsync(user, CancellationToken))
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "LockoutFailed", new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<IdentityResult> AccessFailedAsync(TUser user)
        {
            ...
            Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "LockedUser", new { userId = await GetUserIdAsync(user) });
        ...
        private async Task<IdentityResult> ValidateUserInternal(TUser user)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "UserValidationFailure", new { userId = await GetUserIdAsync(user), errors = string.Join(";", errors.Select(e => e.Code)) });
        ...
        private async Task<IdentityResult> ValidatePasswordInternal(TUser user, string password)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.Log(System.Diagnostics.Tracing.LogLevel.Warning, "PasswordValidationFailure", new { userId = await GetUserIdAsync(user), errors = string.Join(";", errors.Select(e => e.Code)) });
