...
namespace Microsoft.AspNet.Identity
{
    ...
    public class UserManager<TUser> : IDisposable where TUser : class
    {
        ...
        public virtual async Task<bool> CheckPasswordAsync(TUser user, string password)
        {
            ...
            if (!success)
            {
                Logger.Log("PasswordMismatch", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
            }
            return success;
        }
        ...
        public virtual async Task<IdentityResult> AddPasswordAsync(TUser user, string password)
        {
            ...
            if (hash != null)
            {
                Logger.Log("UserAlreadyHasPassword", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<IdentityResult> AddLoginAsync(TUser user, UserLoginInfo login)
        {
            ...
            {
                Logger.Log("LoginAlreadyAssociated", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
        ...
        {
            Logger.Log("UserAlreadyInRole", LogLevel.Warning, new { userId = await GetUserIdAsync(user), role = role });
        ...
        {
            Logger.Log("UserNotInRole", LogLevel.Warning, new { userId = await GetUserIdAsync(user), role = role });
        ...
        public virtual async Task<IdentityResult> ChangePhoneNumberAsync(TUser user, string phoneNumber, string token)
        {
            ...
            {
                Logger.Log("InvalidToken", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
                return IdentityResult.Failed(ErrorDescriber.InvalidToken());
            }
            await store.SetPhoneNumberAsync(user, phoneNumber, CancellationToken);
        ...
        public virtual async Task<bool> VerifyUserTokenAsync(TUser user, string tokenProvider, string purpose, string token)
        {
            ...
            if (!result)
            {
                Logger.Log("InvalidToken", LogLevel.Warning, new { userId = await GetUserIdAsync(user), purpose = purpose });
            }
            return result;
        }
        ...
        public virtual async Task<IdentityResult> SetLockoutEndDateAsync(TUser user, DateTimeOffset? lockoutEnd)
        {
            ...
            if (!await store.GetLockoutEnabledAsync(user, CancellationToken))
            {
                Logger.Log("LockoutFailed", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
        ...
        public virtual async Task<IdentityResult> AccessFailedAsync(TUser user)
        {
            ...
            Logger.Log("LockedUser", LogLevel.Warning, new { userId = await GetUserIdAsync(user) });
        ...
        private async Task<IdentityResult> ValidateUserInternal(TUser user)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.Log("UserValidationFailure", LogLevel.Warning, new { userId = await GetUserIdAsync(user), errors = string.Join(";", errors.Select(e => e.Code)) });
        ...
        private async Task<IdentityResult> ValidatePasswordInternal(TUser user, string password)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.Log("PasswordValidationFailure", LogLevel.Warning, new { userId = await GetUserIdAsync(user), errors = string.Join(";", errors.Select(e => e.Code)) });
                return IdentityResult.Failed(errors.ToArray());
            }
            return IdentityResult.Success;
        }
