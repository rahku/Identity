...
namespace Microsoft.AspNet.Identity
{
    ...
    public class SignInManager<TUser> where TUser : class
    {
        ...
        public virtual async Task<bool> CanSignInAsync(TUser user)
        {
            ...
            {
                Logger.Log("MissingEmail", LogLevel.Warning, new { userId = await UserManager.GetUserIdAsync(user) });
                return false;
            }
            if (Options.SignIn.RequireConfirmedPhoneNumber && !(await UserManager.IsPhoneNumberConfirmedAsync(user)))
            {
                Logger.Log("MissingPhone", LogLevel.Warning, new { userId = await UserManager.GetUserIdAsync(user) });
                return false;
            }
            ...
            bool isPersistent, bool lockoutOnFailure)
        {
            ...
            Logger.Log("PasswordMismatch", LogLevel.Warning, new { userId = await UserManager.GetUserIdAsync(user) });
        ...
        {
            Logger.Log("LockedUser", LogLevel.Warning, new { userId = await UserManager.GetUserIdAsync(user) });
